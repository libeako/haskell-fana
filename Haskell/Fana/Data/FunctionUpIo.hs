...
import Prelude (fmap, Applicative, pure, either)
...
instance Functor f => Profunctor (UpI f) where
	dimap fi fo (UpI f) = UpI (fmap fi >>> f >>> fo)
instance Functor f => Profunctor (UpO f) where
	dimap fi fo (UpO f) = UpO (fi >>> f >>> fmap fo)
...
load_pe_ f (c, i) = fmap (HePair.after   c) (f i)
load_pl_ f (i, c) = fmap (HePair.before  c) (f i)

load_se_ :: Applicative f => (i -> f o) -> (Either c i -> f (Either c o))
load_sl_ :: Applicative f => (i -> f o) -> (Either i c -> f (Either o c))
load_se_ f = either (Left >>> pure) (f >>> fmap Right)
load_sl_ f = either (f >>> fmap Left) (Right >>> pure)
